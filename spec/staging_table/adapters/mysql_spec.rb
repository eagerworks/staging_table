...
RSpec.describe StagingTable::Adapters::Mysql, :mysql do
  ...
  describe "#create_table" do
    ...
    it "includes indexes by default (MySQL LIKE behavior)" do
      adapter.create_table(temp_table_name, "test_users")

      indexes = connection.indexes(temp_table_name)
      # MySQL LIKE copies indexes by default
...
RSpec.describe StagingTable::Adapters::Base, :mysql do
  describe ".for" do
    it "returns a Mysql adapter for mysql connections" do
      adapter = described_class.for(ActiveRecord::Base.connection)
      expect(adapter).to be_a(StagingTable::Adapters::Mysql)
    end
  end
end
