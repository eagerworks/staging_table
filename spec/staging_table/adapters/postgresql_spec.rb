...
RSpec.describe StagingTable::Adapters::Postgresql, :postgresql do
...
RSpec.describe StagingTable::Adapters::Base, :postgresql do
  describe ".for" do
    it "returns a Postgresql adapter for postgresql connections" do
      adapter = described_class.for(ActiveRecord::Base.connection)
      expect(adapter).to be_a(StagingTable::Adapters::Postgresql)
    end
  end
end
